  ...
  return (
    <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
        ...
        <header className="flex gap-4">
          <img className="h-8 w-8 rounded-lg" src={user.avatar_url} alt="User imag" />
          <h2 className="text-2xl font-medium flex items-center gap-2">
            {user.firstName} {user.lastName} <span className="italic text-lg">({user.id})</span>
          </h2>
        </header>
        <div className="flex flex-col gap-2">
          <div>
            <span>Email: </span><a href={`mailto:${user.email}`} className="text-zinc-300 hover:text-zinc-400 transition-colors">{user.email}</a>
          </div>
        ...
        <div className="flex flex-col gap-2 py-2 px-4 rounded-lg bg-slate-700">
          <header className="flex justify-between">
            ...
            <span className="flex items-center justify-center bg-gray-900 text-xs text-zinc-300 rounded-lg h-6 px-2">{user.posts.length}</span>
          </header>
        ...
        <div className="flex flex-col gap-2 py-2 px-4 rounded-lg bg-slate-700">
          <header className="flex justify-between">
            ...
            <span className="flex items-center justify-center bg-gray-900 text-xs text-zinc-300 rounded-lg h-6 px-2">{user.comments.length}</span>
          </header>
          <ul className="flex flex-wrap gap-4">
            {
        ...
        <div className="flex flex-col gap-2 py-2 px-4 rounded-lg bg-slate-700">
          <header className="flex justify-between">
            ...
            <span className="flex items-center justify-center bg-gray-900 text-xs text-zinc-300 rounded-lg h-6 px-2">{user.followers.length}</span>
          </header>
          <ul className="flex flex-wrap gap-4">
            {
