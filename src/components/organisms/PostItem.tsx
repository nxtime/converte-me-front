  ...
  return (
    <>
      <header className="flex h-10 items-center justify-between w-full gap-4">
        <button onClick={() => setIsUserModalOpen(true)} className="flex gap-4 items-center py-2 px-4 rounded-lg bg-transparent hover:bg-fuchsia-500 transition-colors">
          <img className="h-8 w-8 aspect-square rounded-lg" src={post.user.avatar_url} alt={"User post creator image"} />
          {
        ...
        <div className="flex gap-4 items-center">
          <button className="text-lg font-medium text-zinc-100 hover:text-zinc-400 transition-colors" onClick={() => setIsOpen(s => !s)}>{post.title}</button> 
          <button 
      ...
      <UserModal userId={post.user.id} isOpen={isUserModalOpen} setIsOpen={setIsUserModalOpen} />
    </>
  )
}
