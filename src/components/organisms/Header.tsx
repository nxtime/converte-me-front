...
const Header = () => {
  ...
  if (!currentUser()?.id) return (
    <div className="flex justify-between items-center">
      <h2 className="text-2xl font-bold h-16 felx items-center">ConverteMe</h2>
    </div>
  );
    ...
    <header className="flex justify-between items-center">
      ...
      <div className="relative flex flex-col items-center gap-2">
        ...
        >
          ...
          <Icon icon="bxs:down-arrow" className="text-sm" vFlip={isUserMenuOpen} />
        </button>
        {
          isUserMenuOpen && (
            <ul className="w-full absolute top-[calc(100%+0.5rem)] flex flex-col gap-1 bg-gray-900 shadow-md p-1 rounded-lg">
              ...
              <button
                className="flex gap-1 items-center justify-center bg-gray-800 hover:bg-fuchsia-500 transition-colors w-full rounded-lg h-8"
              >
